	...
	<%	// || && 차이
		...
		if( request.getParameter("id") != null &&
			...
			request.getParameter("pw").equals("admin1234"))
				{
			/* Cookie ( Response를 통해서 페이지 전환이 일어났을 때 또는 일정시간 후에 어떤 데이터를
					삭제해야 하는 상황 등등에서 임시로 사용할 수 있는 데이터 저장공간 ) */
			...
			response.sendRedirect("Ex02_Response/Ex02_Success.jsp"); //→ Servlet에서 연결할 때는 폴더명도 써줘야 함
		}else{
			//response.sendRedirect("Ex02_Failed.jsp");
			response.sendRedirect("Ex02_Response/Ex02_Failed.jsp"); //→ Servlet에서 연결할 때는 폴더명도 써줘야 함
		}//if
